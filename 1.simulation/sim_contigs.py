    ...
    with open(fasta) as f:
        for line in f:
            ...
            if line.startswith('>'):
                ...
                lstr = line.strip()
                len_dict[temp_id] += len(lstr)
                seq_dict[temp_id].append(lstr)

    for temp_id in seq_dict:
        seq_dict[temp_id] = ''.join(seq_dict[temp_id])
...
def get_ctg_len_random_list(mean, cv, len_dict, seed, min_len):
    ...
    ctg_len_list = [int(v) for v in scipy.stats.norm.rvs(loc=mean, scale=cv * mean, size=n, random_state=seed) if
                    int(v) >= min_len]
...
def output_fasta(output_ctg_dict, fasta, seq_dict, ctg_ori_list, mean, cv):
...
def main():
    ...
    parser.add_argument('--min_len', type=int, default=5000,
                        help='minimum output contig length, however still some contigs longer than 0.5*mean*(1-CV) in chromosome ends may be output, default: %(default)s bp')
    ...
    output_fasta(output_ctg_dict, args.fasta, seq_dict, ctg_ori_list, args.mean, args.CV)
